...
local function vim_navigate(direction)
  ...
  if not success then
    vim.notify(
      "E11: Invalid in command-line window; <CR> executes, CTRL-C quits: wincmd " .. direction,
      vim.log.levels.WARN
    )
  end
end
...
local function tmux_aware_navigate(direction)
  ...
  if should_forward_navigation(tmux_last_pane, at_tab_page_edge) then
    ...
    if M.config.no_wrap then
      args = string.format('if -F "#{pane_at_%s}" "" "%s"', pane_position_from_direction[direction], args)
    end
...
function M.setup(user_config)
  ...
  if not vim.env.TMUX then
    vim.api.nvim_create_user_command("SplitJumpLeft", function()
      vim_navigate("h")
    end, {})
    vim.api.nvim_create_user_command("SplitJumpDown", function()
      vim_navigate("j")
    end, {})
    vim.api.nvim_create_user_command("SplitJumpUp", function()
      vim_navigate("k")
    end, {})
    vim.api.nvim_create_user_command("SplitJumpRight", function()
      vim_navigate("l")
    end, {})
    vim.api.nvim_create_user_command("SplitJumpPrevious", function()
      vim_navigate("p")
    end, {})
    return
  end
